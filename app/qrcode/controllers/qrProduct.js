vietsoftApp.controller('QRProductCtrl', ['$scope', '$http', '$resource', 'ngAuthSettings', 'qrCodeServices', 'authService',
	function ($scope, $http, $resource, ngAuthSettings, qrCodeServices, authService) {

	    $scope.controllerUrl = ngAuthSettings.apiServiceBaseUri + '/api/QRProducts/';
	    $scope.dialogId = 'qrCodeProductDialog';
	    ...
	    $scope.resetForm = function () {
	        $scope.qrcodeProduct.code = '';
	        $scope.qrcodeProduct.item.code = '';
	        $scope.qrcodeProduct.item.name = '';
	        $scope.qrcodeProduct.item.price = 0;
	        $scope.qrcodeProduct.item.description = '';
	        $scope.qrcodeValidate.code = false;
	        $scope.qrcodeValidate.name = false;
	        $scope.qrcodeValidate.price = false;
	        $scope.qrcodeValidate.description = false;
	        $scope.isValid = true;
	    }
	    ...
	    $scope.showUpdateQRCode = function (id) {
	        ...
	        $scope.qrcodeProduct.item.id = id;
	        $http({ method: 'GET', url: $scope.controllerUrl + id }).
				success(function (data, status, headers, config) {
				    $scope.qrcodeProduct = data;
				    $scope.qrcodeProduct.code = "data:image/png;base64," + data.code;
	    ...
	    $scope.SaveQRCode = function () {
	        ...
	        if ($scope.qrcodeProduct.item.code.length == 0) {
	            $scope.qrcodeValidate.code = true;
	            $scope.isValid = false;
	        }
	        if ($scope.qrcodeProduct.item.name.length == 0) {
	            $scope.qrcodeValidate.name = true;
	            $scope.isValid = false;
	        }
	        if ($scope.qrcodeProduct.item.price.length == 0) {
	            $scope.qrcodeValidate.price = true;
	            $scope.isValid = false;
	        }
	        if ($scope.qrcodeProduct.item.description.length == 0) {
	            $scope.qrcodeValidate.description = true;
	            $scope.isValid = false;
	        }
	        ...
	        if ($scope.isValid) {
	            ...
	            var jsonData = JSON.stringify($scope.qrcodeProduct.item);

	            var id = $scope.qrcodeProduct.item.id;
	            if (id > 0) {
					...
					.then(function (result) {
					    ...
					    if (result.data.code == undefined) {
					        ...
					        $scope.qrcodeProduct.code = "data:image/png;base64," + result.data.code;
					        $scope.listQRCode();
					    }
					},
					function (response) {
	    ...
	    $scope.deleteSelectedQRCode = function () {
	        ...
	        if (isDelete) {
	            var listId = [];
	            $('input[name="qrcodeProduct"]').each(function (index, item) {
